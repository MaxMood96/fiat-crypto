...
y(){return[...b.set].map(a=>a.deref()).filter(a=>a)}var
r;function
n(a,b,c){try{return new
WebAssembly.Function(a,b,c)}catch(f){return b}}const
l=new
TextDecoder("utf-8",{ignoreBOM:1}),z=new
TextEncoder;function
C(a,b){b=Math.imul(b,0xcc9e2d51|0);b=b<<15|b>>>17;b=Math.imul(b,0x1b873593);a^=b;a=a<<13|a>>>19;return(a+(a<<2)|0)+(0xe6546b64|0)|0}function
D(a,b){for(var
c=0;c<b.length;c++)a=C(a,b.charCodeAt(c));return a^b.length}let
u={jstag:WebAssembly.JSTag||new
...
c=0,b=a.length;while(1){let{read:d,written:e}=z.encodeInto(a.slice(c),K);b-=d;if(!b)return e;w(e);c+=d}},ta_create:(a,b)=>new
s[a](b),ta_normalize:a=>a
instanceof
Uint32Array?new
Int32Array(a.buffer,a.byteOffset,a.length):a,ta_kind:b=>s.findIndex(a=>b
instanceof
...
Int32Array(12)),open:(a,d,c)=>f.openSync(a,J.reduce((a,b,c)=>d&1<<c?a|b:a,0),c),close:a=>f.closeSync(a),write:(a,b,c,d,e)=>f?f.writeSync(a,b,c,d,e==null?e:Number(e)):(console[a==2?"error":"log"](typeof
b=="string"?b:l.decode(b.slice(c,c+d))),d),read:(a,b,c,d,e)=>f.readSync(a,b,c,d,e),file_size:a=>f.fstatSync(a,{bigint:true}).size,register_channel:L,unregister_channel:N,channel_list:y,exit:a=>h&&d.exit(a),argv:()=>h?d.argv.slice(1):["a.out"],getenv:a=>h?d.env[a]:null,system:a=>{var
b=require("child_process").spawnSync(a,{shell:true,stdio:"inherit"});if(b.error)throw b.error;return b.signal?255:b.status},time:()=>performance.now(),getcwd:()=>h?d.cwd():"/static",chdir:a=>d.chdir(a),mkdir:(a,b)=>f.mkdirSync(a,b),unlink:a=>f.unlinkSync(a),readdir:a=>f.readdirSync(a),file_exists:a=>+f.existsSync(a),rename:(a,b)=>f.renameSync(a,b),throw:a=>{throw a},start_fiber:a=>r(a),suspend_fiber:n({parameters:["externref","funcref","eqref"],results:["eqref"]},(c,b)=>new
...
a==="string"),compare:(a,b)=>a<b?-1:+(a>b),hash:D,decodeStringFromUTF8Array:()=>"",encodeStringToUTF8Array:()=>0};const
g=Object.assign({Math:I,bindings:u,js:t,"wasm:js-string":m,"wasm:text-decoder":m,"wasm:text-encoder":m,env:{}},B),q={builtins:["js-string","text-decoder","text-encoder"]};function
H(a){const
...
G=h?H:A;async function
F(a){return h?WebAssembly.instantiate(await
a,g,q):WebAssembly.instantiateStreaming(a,g,q)}async function
E(){g.OCaml={};const
c=[];async function
b(a,b){const
f=a[1].constructor!==Array;async function
e(){const
d=G(M+"/"+a[0]+".wasm");await
Promise.all(f?c:a[1].map(a=>c[a]));const
e=await
F(d);Object.assign(b?g.env:g.OCaml,e.instance.exports)}const
...
O=await
E();var{caml_callback:c,caml_alloc_tm:o,caml_start_fiber:x,caml_handle_uncaught_exception:p,caml_buffer:v,caml_extract_string:w,_initialize:k}=O.instance.exports,i=v?.buffer,K=i&&new
Uint8Array(i,0,i.length);r=n({parameters:["eqref"],results:["externref"]},x,{promising:"first"});var
...
k()})(function(a){"use strict";return{}}(globalThis))({"link":[["code-ea47bccdfaf909a29649",0]],"generated":(a=>{var
